import cv2, time, logging, os, sys, argparse
...
DB_CONFIG = {
    'host': 'plate.pikselanalitik.com',
    'user': 'pikselan_plate',
    'password': 'KWw7m#]mid4O@Gt-',
    'database': 'pikselan_plate'
}
API_URL = "http://plate.pikselanalitik.com/api/log_event.php"
GATE_SERVO_PINS = { 'giris': 18, 'cikis': 18 } # İki kapı da GPIO 18'i tetikler
# -----------------
...
def main():
    ...
    try:
        ...
        engine = LPREngine(detector_model_path=MODEL_PATH, db_config=DB_CONFIG, servo_pin=servo_pin)
    ...
    while True:
        try:
            if cap is None or not cap.isOpened():
                ...
                if not cap.isOpened():
                    logger.error("Kameraya bağlanılamadı. 10s sonra denenecek..."); time.sleep(10)
                    continue
                logger.info("Kamera bağlantısı başarılı.")
            ret, frame = cap.read()
            if not ret:
                logger.warning("Kare okunamadı. Yeniden bağlanılıyor..."); cap.release(); cap = None; time.sleep(5)
                continue
            ...
            if (current_time - last_process_time) > PROCESS_INTERVAL_SECONDS:
                ...
                else: logger.info("Bu karede plaka bulunamadı.")
            time.sleep(0.01)
